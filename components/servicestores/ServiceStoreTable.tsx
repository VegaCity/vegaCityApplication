...
import { StoreService, ServiceResponse } from "@/types/serviceStore";
import { StoreServiceServices } from "../services/storeServiceServices";
...
const ServiceStoreTable = () => {
  ...
  const fetchStores = async () => {
    try {
      const response = await StoreServiceServices.getStores({
        page: currentPage,
        size: 10,
      });
  ...
  const handleDelete = async (id: string) => {
    try {
      await StoreServiceServices.deleteStoreById(id);
      fetchStores();
        ...
        <TableBody>
          {stores.map((store, i) => (
            <TableRow key={store.id}>
              ...
              <TableCell>{store.deflag ? "Yes" : "No"}</TableCell>
              <TableCell>
                <Link href={`/admin/serviceStores/edit/${store.id}`}>
                  <Button
                    variant="outline"
                    size="sm"
...
export default ServiceStoreTable;
