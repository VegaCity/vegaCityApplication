...
const ZoneTable = ({ limit, title }: PackageTableProps) => {
  ...
  useEffect(() => {
    ...
    const fetchZones = async () => {
      try {
        ...
        const zones = Array.isArray(response.data.items) ? response.data.items : [];
        setZoneList(zones);
      } catch (err) {
  ...
  const handleDeleteZone = (zone: ZoneType) => {
    ...
    if(zone.id){
      ZoneServices.deleteZoneById(zone.id).then((res) => {
                ...
                <TableCell>
                  <Link href={`/admin/zones/edit/${zns.id}`}>
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>Are sure for delete this -{zns?.name}- ZONE?</AlertDialogTitle>
                          <AlertDialogDescription>
                            This action cannot be undone. This will deflag Zone in your Zone list!
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>Cancel</AlertDialogCancel>
                          <AlertDialogAction onClick={() => handleDeleteZone(zns)}>Confirm</AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                    </AlertDialog>
                </TableCell>
